...
# its checks reads ui/dist — the harness's just-built stylesheet — and
# cross-checks it against ui/src/tokens.css in both directions; a second,
# independent check compares ui/src/tokens.css's own source against itself
# and needs no build at all (see that script's header for what each check
# catches that the other can't). It needs a fresh build, not build-ts's:
