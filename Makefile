...
# real resolver before `build:harness` runs, so a broken map reports as a
# map error rather than a Vite resolve failure — see
# ui/scripts/check-exports.mjs for exactly what it asserts and does not.
