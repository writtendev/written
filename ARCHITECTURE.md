...
### 6. Navigation model: a screen stack
- **Decision:** Written navigates with a single screen stack — activating a row pushes a new screen, a pop action returns to the previous one, and the root model is the one owner of "which screen is current" (its top frame). One global jump (the command palette's quick-switch commands) and direct keys to the top-level lists reset the stack to one of the three top-level screens (Inbox, Review list, Issue list) — nowhere else, and never a push onto whatever stack already existed; searching the palette for a specific object and opening it is a push like any other, not a jump — see `docs/design/screen-map.md` for what a jump does to the stack. A jumped-to Review list or Issue list is itself the stack's root from that point on, the same role Inbox plays at launch, and `esc` at whichever screen is currently the root is a no-op — there is nothing beneath a root to pop to, and it does not fall back to Inbox merely because Inbox is the usual root. The actual key bindings, including which key pops, are `WRTN-5`'s call.
- **Rationale:** Tabbed panes invite unbounded tab management and a second focus concept alongside the stack; a fixed two-pane list/detail layout spends width the diff viewer needs and collapses badly below 100 columns. A push/pop stack gives the current screen full width and degrades to one screen at a time, which is what narrow terminals need. See `docs/design/screen-map.md` for the full navigation diagram, the persistent-chrome decision (a top context bar and bottom hint bar — no permanent sidebar), and the wireframes. `WRTN-9` implements this model in Go.

### 7. The SDLC vocabulary lives in `internal/schema` as a ported `writ.schema`
- **Decision:** Writ is becoming a schema layer with no SDLC vocabulary of its own; the `review`/`comment`/`issue`/`project`/`cycle`/`document`/`section`/`label`/`workflow-state`/`settings` vocabulary that used to live in writ's spec now lives here, as `internal/schema/writ.schema` — a `writ` schema-source file `//go:embed`-ed by a package that imports nothing else from writ, so `make build` never links the engine. The design reasoning and a section-by-section audit of where every piece of the deleted spec landed are in `docs/sdlc-vocabulary.md`.
- **Rationale:** The vocabulary is written's, not writ's — writ names no downstream product and written is an ordinary consumer of writ's public schema-source grammar. `internal/` is where a `//go:embed` pattern can reach and where the Makefile's `check-go-packages` guard requires Go files to live; the repository root would wrongly assert "this repo's vocabulary" when the file is what written ships into its *users'* repositories, and `docs/` is for prose, not a gated machine artifact. The acceptance gate is an ordinary `internal/schema` Go test (`make check-go`'s existing `./internal/...` root already covers it) that parses, compiles, and folds the schema exactly as `writ schema plan` does against an empty repository, and asserts every vendored `field-rules.json` rule against the compiled result — turning the port's audit from trusted into tested. The one dependency this adds, `github.com/writtendev/writ`, is test-only.
