...
- **Decision:** Written navigates with a single screen stack — activating a row pushes a new screen, `esc` pops it, and the root model is the one owner of "which screen is current" (its top frame). One global jump (the command palette) and direct keys to the top-level lists reach any screen without unwinding the stack first.
