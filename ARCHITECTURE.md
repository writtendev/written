...
- **Decision:** Written navigates with a single screen stack — activating a row pushes a new screen, a pop action returns to the previous one, and the root model is the one owner of "which screen is current" (its top frame). One global jump (the command palette) and direct keys to the top-level lists reset the stack to reach any screen directly, without unwinding it first — see `docs/design/screen-map.md` for what a jump does to the stack. The actual key bindings, including which key pops, are `WRTN-5`'s call.
